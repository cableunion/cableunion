...
import os.path
from common.models.SysSettings import ZLLMSysSettings
...
class smtpMail(object):
    def __init__(self, From, from_smtp, from_user, from_passrd):
        self.From = From
    ...
    def send_mail(self, tomail=None, subject=None, content=None, file_name=None, user_uuid=None):
        ...
        #添加html的邮件内容
        txt = MIMEText("请点击以下链接或者将地址复制到浏览器完成注册。<br/><a href='http://www.xn--fiqz50h4nfglb.com/account/register/{0}'>www.中联联盟.com/{1}</a><br/>中缆联盟期待您的加入！！！".format(user_uuid, user_uuid), _subtype='html',  _charset='UTF-8')
        msg.attach(txt)
